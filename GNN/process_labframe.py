...
graphsdir = "../graphdata/new_Input_Polarisations_ZZ__16th_November2024_lab.h5" #directory to store graph data 
...
class MyOwnDataset(Dataset):
    ...
    def _preprocess(self, column, i):
        ...
        E_const = column[12] * np.cosh(column[9]) #energy of constituent  pT*cosh(eta)
        log_E_const = np.log(E_const)
        log_pT = np.log(column[12]) #log particle transverse momentum
        ...
        rel_pT = np.log(column[12] / p_ref) #log(pT/pT(lepton/jet))
        delta_eta = column[9] - eta_ref #difference in eta between constituent and lepton/jet
        delta_phi = column[11] - phi_ref #difference in phi 
